...
function main_menu() {
	...
	dialog --backtitle "RetroPie BGM Player v.$bgm_curversion" --title "BGM Updater" --ok-label "Check" --extra-button --extra-label "Reinstall" --yesno "${infobox}" 0 0
	
	opt=$?
	[ $opt -eq 1 ] && exit
	[ $opt -eq 3 ] && updateinstall
